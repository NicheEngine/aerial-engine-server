...
import io.github.nicheengine.aerial.error.AerialErrorCode;
...
public class MqttErrorCode implements AerialErrorInfo {
    private static final int MOD = 100_000;

    private ErrorSource source;
    private AerialErrorCode errorCode;
    private boolean success;
    private Integer sourceCode;

    @JsonCreator
    public MqttErrorCode(Integer errorCode) {
        this.sourceCode = errorCode;
        if (MqttResultReply.CODE_SUCCESS == errorCode) {
            this.success = true;
            this.errorCode = AerialErrorStatus.AERIAL_SUCCESS;
        } else {
